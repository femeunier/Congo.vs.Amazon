...
options(timeout = max(6000,
                      getOption("timeout")))

# Inputs

outfolder <- "/home/carya/output/dbfiles/Test"
outfolder <- "/data/gent/vo/000/gvo00074/ED_common_data/met/CRUNCEP/Tropics/"
...
for (i in seq(1,length(years))){
  ...
  for (j in seq(1,length(var$DAP.name))){
    ...
    tmp_file <- tempfile()
    utils::download.file(ncss_query,
                         tmp_file)
  ...
  dat.list[[3]] <- dat.list[[3]]/21600

  loc <- ncdf4::nc_create(filename = loc.file,
                          vars = var.list,
                          verbose = verbose)
...
# scp /home/femeunier/Documents/projects/Congo.vs.Amazon/scripts/Download.CRUNCEP.R hpc:/kyukon/data/gent/vo/000/gvo00074/felicien/R
