...
sigma_D_main_all = seq(0.1,1,0.15) # model performance
sigma_S_main_all = seq(0.1,1,0.3) # model similarities

data.selected <- readRDS("./outputs/data.selected.RDS")
model.selected <- readRDS("./outputs/model.selected.RDS") %>%
  filter(model %in% models.selection)
Snorm <- readRDS("./outputs/Data.vs.Model.RDS") %>%
  filter(cimodel %in% c(models.selection,"CRUJRA"),
         cjmodel %in% c(models.selection,"CRUJRA")) %>%
  filter(variable %in% vars2test)

df.Acc.all <- data.frame() ; df.rsq <- data.frame() ; df.N <- data.frame()
