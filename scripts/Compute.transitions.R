...
for (ifile in seq(1,length(files))){
  ...
  A.sum <- A.cat %>%
    dplyr::select(lon,lat,year,month,scenario,model,biome,MAP,MCWD)
  ...
  A.cat.dt[, paste0("biome.", 1:7) := lapply(1:7, function(x) roll_freq(biome, x, 360)),
           by = .(model, lon, lat)]

  A.cat.dt.lon <- melt(
    A.cat.dt[, !"biome"],                           # Drop original `cat` column
    measure.vars = patterns("^biome\\.[1234567]$"), # Select cat.1, cat.2, cat.3
    variable.name = "biome",                        # Name for new 'cat' column
    value.name = "value"                            # Name for values
  )


  A.cat.dt.lon[, biome := as.integer(gsub("biome\\.", "", biome))]
  ...
  ), by = .(year, month, end,biome)]

  # ggplot(data = A.cat.dt.lon.sum.forests) +
  #   geom_line(aes(x = year + (month - 1/2)/12,
  #                 y = MAP.m, color = as.factor(end))) +
  #   scale_color_brewer(direction = -1,) +  # or use "Dark2", "Paired", etc.
  #   theme_bw()
  #
  # ggplot(data = A.cat.dt.lon.sum.forests) +
  #   geom_line(aes(x = year + (month - 1/2)/12,
  #                 y = MCWD.m, color = as.factor(end))) +
  #   scale_color_brewer(direction = -1,) +  # or use "Dark2", "Paired", etc.
  #   theme_bw()
  #
  # ggplot(data = A.cat.dt.lon.sum.forests) +
  #   geom_line(aes(x = year + (month - 1/2)/12,
  #                 y = value.m, color = as.factor(biome))) +
  #   scale_color_brewer(direction = -1,) +  # or use "Dark2", "Paired", etc.
  #   facet_wrap(~ end) +
  #   theme_bw()

  # all.transitions <- bind_rows(all.transitions,
  #                          A.cat.dt.lon.sum.forests %>%
  #                            mutate(model = cmodel,
  #                                   scenario = cscenario))
...
# scp /home/femeunier/Documents/projects/Congo.vs.Amazon/scripts/Compute.transitions.R hpc:/data/gent/vo/000/gvo00074/felicien/R/
