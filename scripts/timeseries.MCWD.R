...
models <- rev(sort(unique(df.files.wide[["model"]])))
...
for (cmodel in models){
  ...
  OP.file <- paste0("./outputs/Timeseries.MCWD.",cmodel,".RDS")
  ...
  for (irow in seq(1,nrow(cdf.files.wide))){
    ...
    for (ivar in seq(1,length(cvars))){
      ...
      if (!all(file.exists(cfile))){
        next()
      } else{
        cdf.tempA <- bind_rows(readRDS(cfile))
      }
    ...
    for (cperiod in seq(1900,2100,1)){

      print(cperiod)

      ctemp <- cdata.all %>%
        filter(period %in% cperiod)
...
# scp /home/femeunier/Documents/projects/Congo.vs.Amazon/scripts/timeseries.MCWD.R hpc:/kyukon/data/gent/vo/000/gvo00074/felicien/R
