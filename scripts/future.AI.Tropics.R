...
saveRDS(df.all.selected,
        "./outputs/All.model.scenario.RDS")
...
ggplot(data = df.weights %>%
         filter(model != "CRUJRA")) +
  geom_density(aes(x = w), fill = "grey",
               alpha = 0.5, color = NA) +
  theme_bw()
...
df.all.selected.weights %>%
  ...
  summarise(w.tot = sum(w,na.rm = TRUE)/length(unique(lon.lat)),
...
df.diff.temp <- df.all.sum %>%
  mutate(reference = case_when(period == 2000 ~ "yes",
                               TRUE ~ "no")) %>%
...
df.diff <- df.diff.temp %>%
  ...
  left_join(df.diff.temp %>%
              filter(reference == "yes",
                     period == 2000) %>%
              rename(obs = value) %>%
              dplyr::select(-c("period","scenario","period")),
            by = c("lon","lat","variable","weighting")) %>%
  filter(!is.na(obs)) %>%
  group_by(period,scenario,weighting,variable) %>%
  mutate(diff = obs - mod)
...
df.transitions <- df.all.sum %>%
  group_by(weighting,period,scenario) %>%
  summarise(trop = sum(biome == "Humid_large"),
            .groups = "keep") %>%
  ungroup() %>%
  left_join(df.GW.sum,
            by = c("scenario","period")) %>%
  group_by(weighting) %>%
  mutate(trop.rel = trop/mean(trop[period <= 1930]))
...
df2augment <- df.all.augment <- data.frame()
for (cscenario in cscenarios){
  ...
  df.all.augment <- bind_rows(df.all.augment,
                              df.all.selected %>%
                                filter(scenario == "historical") %>%
                                mutate(scenario = cscenario) %>%
                                distinct(),
...
saveRDS(df2augment,
        "./outputs/df.climate.change.Tropics.RDS")
saveRDS(df.all.augment,
        "./outputs/df.all.climate.change.Tropics.RDS")
...
df2augment.sum <- df2augment %>%
  ...
  filter(period >= 2000) %>%
  group_by(weighting,scenario,period,biome) %>%
  summarise(N = n(),
            .groups = "keep") %>%
  group_by(weighting,scenario,period) %>%
  mutate(r = N/sum(N))
...
df.transitions <- df2augment %>%
  ...
  #                                                  "near_future","mid_future","long_future")))) %>%
  # filter(period >= 2000) %>%
  dplyr::select(period,scenario,lat,lon,weighting,biome,biome.num) %>%
  group_by(scenario,lat,lon,weighting) %>%
  arrange(period) %>%
...
saveRDS(df.transitions.sum,
        "./outputs/Transitions.vs.GW.RDS")
